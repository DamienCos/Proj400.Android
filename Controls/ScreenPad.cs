...
namespace AndroidTest
{
    public class ScreenPad
    {
        ...
        private void PlaceControls(bool xType)
        {
            ...
            yOffest = Game1.ScreenSize.Y - padTexture.Height - 130;
        ...
        public void Update()
        {
            ...
            if (xTouch.HasValue)
            {
                xPressed = VirtualButtonState.Pressed;
                xColor = Color.White;
            }
            else
            {
                xId = -1;
                xPressed = VirtualButtonState.Released;
                xColor = Color.LightGray;
            }
            GetButtonStateA(xPressed);
            #endregion

            #region y Touch
            if (yTouch.HasValue)
            {
                yPressed = VirtualButtonState.Pressed;
                yColor = Color.White;
            }
            else
            {
                yId = -1;
                yPressed = VirtualButtonState.Released;
                yColor = Color.LightGray;
            }
            GetButtonStateY(yPressed);
            #endregion

            #region a Touch
            if (aTouch.HasValue)
            {
                aPressed = VirtualButtonState.Pressed;
                aColor = Color.White;
            }
            else
            {
                aId = -1;
                aPressed = VirtualButtonState.Released;
                aColor = Color.LightGray;
            }
            GetButtonStateA(aPressed);
            #endregion

            #region b Touch
            if (bTouch.HasValue)
            {
                bPressed = VirtualButtonState.Pressed;
                bColor = Color.White;
            }
            else
            {
                bId = -1;
                bPressed = VirtualButtonState.Released;
                bColor = Color.Yellow;
            }
            GetButtonStateB(bPressed);
            #endregion
#endif
        }

        public VirtualButtonState GetButtonStateA(VirtualButtonState a)
        {
            return a;
        }

        public VirtualButtonState GetButtonStateB(VirtualButtonState b)
        {
            return b;
        }

        public VirtualButtonState GetButtonStateX(VirtualButtonState x)
        {
            return x;
        }

        public VirtualButtonState GetButtonStateY(VirtualButtonState y)
        {
            return y;
        }
